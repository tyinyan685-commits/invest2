...
export function ResearchMethodReport({ analysis }: { analysis: ResearchOutput }) {
  ...
  const analystCards = [
    ["基本面分析师", analysis.analysts.fundamentals.judgment, [analysis.analysts.fundamentals.quality, analysis.analysts.fundamentals.earningsTrend, analysis.analysts.fundamentals.valuation, analysis.analysts.fundamentals.cashFlowAndBalanceSheet]],
    ...
    ["新闻分析师", analysis.analysts.news.judgment, [...analysis.analysts.news.companyEvents, ...analysis.analysts.news.nextCatalysts]],
  ...
  return <div className="method-report">
    <section className="method-decision">
      ...
      <div><span className="section-kicker">FINAL DECISION</span><h2>{analysis.executiveSummary.oneSentence}</h2><p>{analysis.portfolioManager.ratingReason}</p><div className="decision-action"><Target size={16} /><b>{analysis.executiveSummary.positionAction}</b><span>{analysis.portfolioManager.eventStance}</span></div></div>
    </section>
    ...
    <section className="method-section">
      ...
      <div className="plan-notes"><p><b>事件规则</b>{analysis.tradePlan.eventRules.join("；")}</p><p><b>逻辑失效</b>{analysis.tradePlan.invalidation}</p><p><b>风险收益</b>{analysis.tradePlan.riskReward}</p></div>
    </section>

    <section className="method-section">
      <div className="method-heading"><span className="section-kicker">FROZEN ANALYST REPORTS</span><h2>四份冻结分析师报告</h2></div>
      <div className="analyst-cards">{analystCards.map(([name, judgment, details]) => <article key={name}><span>{name}</span><h3>{judgment}</h3>{details.filter(Boolean).slice(0, 5).map((detail, index) => <p key={`${name}-${index}`}>{detail}</p>)}</article>)}</div>
