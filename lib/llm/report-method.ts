...
4. 每个核心观点必须给 evidenceIds。可用：fmp:quote、fmp:aftermarket、fmp:realtime、fmp:intraday、fmp:profile、fmp:financials、fmp:estimates、fmp:technicals、fmp:insiders、fmp:filings、fmp:filing-text、fmp:earnings、fmp:macro、fmp:sentiment、longbridge:quote、news:N。
...
10. 当前价校准优先级：marketData.longbridge.quote.current（券商实时/扩展盘代理） > marketData.realtime.current > marketData.aftermarket.quote/trade > marketData.quote。必须用 marketData.realtime.intraday5Min.indicators 约束交易建议，写入 marketState；技术指标的大级别支撑阻力仍以 FMP 日线为准。
...
枚举只能从这些值中选择：rating = buy-research/hold/avoid/needs-checking；strength = strong/medium/weak；severity = high/medium/low；evidenceStrength = strong/medium/weak/needs-checking；finalRating = BUY/HOLD/SELL/WATCH；status = success/degraded/missing。
{
  ...
  "marketState": {
    "priceSource": "fmp:realtime/fmp:aftermarket/fmp:quote/longbridge:quote",
    "asOf": "中文时间或 ISO 时间",
    "session": "常规/盘前/盘后/夜盘/未知",
    "currentPrice": null,
    "changePct": null,
    "intradayTrend": "中文",
    "momentum": "中文",
    "volumeState": "中文",
    "triggerSummary": "中文",
    "evidenceIds": ["fmp:realtime", "fmp:intraday"]
  },
  "analysts": {
