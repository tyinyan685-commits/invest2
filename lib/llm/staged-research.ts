...
function base(symbol: string, snapshot: unknown) {
  return `研究标的：${symbol}\n冻结数据快照：${JSON.stringify(snapshot)}\n\n证据 ID 只能使用 fmp:quote、fmp:profile、fmp:financials、fmp:technicals、fmp:insiders、fmp:filings、fmp:earnings、fmp:macro、fmp:sentiment、news:N。价格、指标、财务和事件必须来自快照；缺失数据要直说。每个字段都应写出充分推理，不要把详细报告压缩成一句话。`;
}
...
export async function runResearchStage(stage: ResearchStageName, symbol: string, snapshot: unknown, stages: StoredStages) {
  ...
  switch (stage) {
    ...
    case "sentiment":
      return generateStagedObject("sentiment_report", sentimentStageSchema, system, `${frozen}\n\n你是情绪分析师。评估情绪方向、拥挤度、价格行为与可用社交样本的证据强度。没有期权、空头或可靠社交样本时必须把 evidenceStrength 降为 needs-checking 或 weak，明确样本局限，严禁编造 IV、空头比例、帖子数或机构仓位。`);
    case "debate":
    ...
    case "execution":
      return generateStagedObject("execution_and_risk", executionStageSchema, system, `${frozen}\n\n冻结分析与研究经理裁决：${JSON.stringify({ analysts: { fundamentals: stages.fundamentals, technical: stages.technical, news: stages.news, sentiment: stages.sentiment }, debate: stages.debate })}\n\n你先作为交易员制定详细的条件化执行计划，再由激进、中性、保守三位风险委员分别压力测试。计划必须包含当前姿态、分批进入/加减仓条件、事件前后规则、上下行参考、风险收益与逻辑失效条件。allocationPct 仅指计划仓位内部比例；priceLevel 只能引用冻结数据，无法确认则 null。不要保证收益。`);
    case "portfolio":
      return generateStagedObject("portfolio_decision", portfolioStageSchema, system, `${frozen}\n\n完整研究流程产物：${JSON.stringify(stages)}\n\n你是组合经理，负责最终汇总而不是重新发明事实。给出 BUY/HOLD/SELL/WATCH 条件化评级、目标计划仓位描述、事件姿态、价格框架和主要风险。executiveSummary 先给结论与行动，再列 3-5 条核心理由和 3-6 个风险触发；同时生成完整的快速摘要字段。facts 至少 6 条，sections 必须覆盖基本面、技术面、新闻宏观、情绪四部分，scenarios 至少包含乐观/基准/悲观三种，dataStatus 逐类披露成功、降级或缺失。所有内容应与前述角色结论一致。`);
  }
}
