...
export async function runResearchStage(stage: ResearchStageName, symbol: string, snapshot: unknown, stages: StoredStages) {
  ...
  switch (stage) {
    ...
    case "technical":
      return generateStagedObject("technical_report", technicalStageSchema, system, `${frozen}\n\n你是市场与技术分析师。先读取 snapshot.marketData.realtime.current 和 snapshot.marketData.realtime.intraday5Min.indicators，判断当前价、盘前/盘后价、5分钟 EMA10/EMA20、RSI14、MACD、VWAP、量比是否触发追高/回踩/转弱；若 current.source 为 fmp:aftermarket_delayed 或 current.isFresh=false，必须明确这是延迟扩展盘参考价，不能当实时盘前价。再逐项解释日线 EMA10、SMA50、SMA200、MACD、RSI14、布林带、ATR14、VWMA20，并结合量比、阶段高低点判断趋势、动量、波动和量价。给出四个最重要的支撑/阻力；price 只能使用快照中的价格或由指标直接得到，无法确认则为 null。`);
    case "news":
